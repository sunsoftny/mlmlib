...
var $mValidations = array();
	...
	function PerformValidation($location)
	{
		...
		{
			$ErrFound=0;
			//$tmparray = array_combine(explode("/",$this->mCheckArray[$i]),explode("/",$this->mMessageArray[$i]));
			
			$tmpcheck = explode("/",$this->mCheckArray[$i]);
			$tmpmessage = explode("/",$this->mMessageArray[$i]);
			$tmparray = array();
			...
			if(!$ErrFound && strpos($this->mCheckArray[$i],"checkfile") !== false)	
			{
				...
				{
					$selecterrormsg = explode("-",$tmparray["checkfile:$extension-$size"]);
			...
			if(!$ErrFound && strpos($this->mCheckArray[$i],"datecheck") !== false)
			{
				...
				if($this->IsInvalidDate(explode("-",$dat)))
				{
